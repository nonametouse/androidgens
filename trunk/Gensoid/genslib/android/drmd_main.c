...
static void Genesis_Init(void)
{
  ...
  Detect_Country_Genesis(&Game_Mode, &CPU_Mode);
...
int drmdLoadRom(const char *filename)
{
	...
	if (check_zip(filename)) {
		...
		if (!load_archive(filename, RomData, &RomSize))
			return 0;
	} else {
...
int drmdSaveState(const char *filename)
{
	...
	savestate_mem(&state);
	int rv = save_archive(filename, (char *) &state, sizeof(state));
	sync();
	return rv;
}
...
int drmdLoadState(const char *filename)
{
	...
	size = sizeof(state);
	if (!load_archive(filename, (char *) &state, &size))
		return 0;
